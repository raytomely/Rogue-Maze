...
int maxrow = 27;//23;24		/* Last Line used for map */
int LINES=28, COLS=50;//LINES=25, COLS=80      /* Globals for curses */

SDL_Rect tile_rect = {0, 0, TILE_SIZE, TILE_SIZE};
SDL_Rect background_rect = {0, 0, MAXCOLS*TILE_SIZE, (MAXLINES-3)*TILE_SIZE};
SDL_Rect background_rect_ascii = {0, 0, MAXCOLS*CHAR_WIDTH, (MAXLINES-3)*CHAR_HEIGHT};
...
void render_rooms_old(SDL_Surface *screen)
{
...
{
    int x, y;
    //SDL_FillRect(screen, &background_rect_ascii, SDL_MapRGB(screen->format, 17, 206, 112));
    ...
    for(y = 0; y < MAXLINES-3; y++)
    {
        for(x = 0; x < MAXCOLS; x ++)
        {
            tile_rect.x = x*CHAR_WIDTH; tile_rect.y = y*CHAR_HEIGHT;
            switch(chat(y,x))
            {
                case VWALL:
                case HWALL:
