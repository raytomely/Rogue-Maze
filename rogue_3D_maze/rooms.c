...
void render_rooms(SDL_Surface *screen)
{
    ...
    for(y = 0; y < MAXLINES-3; y++)
    {
        ...
        for(x = 0; x < MAXCOLS; x ++)
        {
            tile_rect.x = x*5; tile_rect.y = y*5;//gl_test_old();
            ...
            switch(chat(y,x))
            {
                ...
                case URWALL:
                case LLWALL:
                case LRWALL:gl_wall();break;//gl_cube();break;
                case FLOOR:gl_floor();break;//{GLubyte color[3]={255,0,0};gl_color_quad(color);break;}
...
void render_rooms_old(SDL_Surface *screen)
{
    ...
    gl_test_old();gl_cube();//gl_test_old();
